...
module.exports = {
   ...
   if(ress && ress.Prefix)prefix = ress.Prefix;
    ...
    if (message.content.match(mention)) {
      const embed = new MessageEmbed()
        ...
        .setDescription(`**Prefix: \`${prefix}\`**\n**Developer: \`gzR41N#9332, Aurel#3353\`**\n**Servers: \`${client.guilds.cache.size}\`**\n \n**Support Server:** If You Need Help Related Me Join My [Support Server](https://discord.gg/UybeVq66hF)
   ...
   if (!channel.permissionsFor(message.guild.me)?.has(Permissions.FLAGS.EMBED_LINKS) && client.user.id !== userId) {
    ...
    if (command.owner && message.author.id !== `${client.owner}`) {
        embed.setDescription("Only <@755566952449310842> And <@915956372204552233> can use this command!");
        return message.channel.send({embeds: [embed]});
    }
